...
class ImpulseSessionOpenConfig:
    timeframe: str
    atr_period: int
    impulse_bars: int
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "ImpulseSessionOpenConfig":
        return cls(**data)


class ImpulseSessionOpenStrategy(BaseStrategy):
    DEFAULT_SYMBOL = "EURUSD"

    def __init__(self, config: ImpulseSessionOpenConfig) -> None:
        self.config = config
    ...
    def _in_session_open_window(self, timestamp: pd.Timestamp) -> bool:
    ...
    def generate_order(
        ...
        if self.config.one_trade_per_day and self._traded_today:
            return None
        if not self._in_session_open_window(timestamp):
            return None
        if atr <= 0:
            return None
