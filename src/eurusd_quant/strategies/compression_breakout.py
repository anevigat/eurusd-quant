...
class CompressionBreakoutConfig:
    ...
    compression_breakout_lookback_bars: int
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "CompressionBreakoutConfig":
        return cls(**data)
...
class CompressionBreakoutStrategy(BaseStrategy):
    ...
    def __init__(self, config: CompressionBreakoutConfig) -> None:
        self.config = config
        ...
        if config.compression_breakout_lookback_bars < 2:
            raise ValueError("compression_breakout_lookback_bars must be >= 2")
    ...
    def _update_range_buffers(self, bar: pd.Series) -> None:
        ...
        if len(self._high_buffer) > self.config.compression_breakout_lookback_bars:
    ...
    def generate_order(
        ...
        if len(self._atr_values) < self.config.compression_median_lookback_bars:
            return None
        if len(prior_highs) < self.config.compression_breakout_lookback_bars:
            return None
