...
class CompressionBreakoutContinuationConfig:
    timeframe: str
    atr_period: int
    compression_median_lookback_bars: int
    breakout_lookback_bars: int
    compression_threshold: float
    breakout_close_strength_threshold: float
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "CompressionBreakoutContinuationConfig":
        return cls(**data)


class CompressionBreakoutContinuationStrategy(BaseStrategy):
    DEFAULT_SYMBOL = "EURUSD"

    def __init__(self, config: CompressionBreakoutContinuationConfig) -> None:
        self.config = config
        if config.atr_period < 1:
            raise ValueError("atr_period must be >= 1")
        if config.compression_median_lookback_bars < 2:
            raise ValueError("compression_median_lookback_bars must be >= 2")
        ...
        if config.breakout_close_strength_threshold <= 0 or config.breakout_close_strength_threshold >= 1:
            raise ValueError("breakout_close_strength_threshold must be in (0, 1)")
    ...
    def _update_atr(self, bar: pd.Series) -> float:
        ...
        if len(self._atr_values) > self.config.compression_median_lookback_bars:
            self._atr_values.pop(0)
        return atr

    def _update_range_buffers(self, bar: pd.Series) -> None:
    ...
    def generate_order(
        ...
        self._update_range_buffers(bar)
        ...
        rolling_median = float(np.median(self._atr_values))
        if rolling_median <= 0:
            return None
        compression_ratio = atr / rolling_median
        ...
        close_strength = (mid_close - mid_low) / bar_range
        ...
        strength_threshold = self.config.breakout_close_strength_threshold

        if mid_close > breakout_high and close_strength >= strength_threshold:
        ...
        if mid_close < breakout_low and close_strength <= (1.0 - strength_threshold):
