...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run NY impulse threshold robustness experiments (p85/p90/p95)."
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output-root",
        default="outputs/ny_impulse_threshold_robustness",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.50,
        help="Fixed retracement entry ratio for all runs",
    )
    parser.add_argument(
        "--p85-price-threshold",
        type=float,
        default=DEFAULT_THRESHOLDS_PRICE["p85"],
        help="p85 impulse threshold in price units",
    )
    parser.add_argument(
        "--p90-price-threshold",
        type=float,
    ...
    parser.add_argument(
        "--p95-price-threshold",
        type=float,
        default=DEFAULT_THRESHOLDS_PRICE["p95"],
        help="p95 impulse threshold in price units",
    )
...
def main() -> None:
    ...
    thresholds_price = {
        "p85": float(args.p85_price_threshold),
        "p90": float(args.p90_price_threshold),
        "p95": float(args.p95_price_threshold),
    }

    results: list[dict] = []
    print("threshold | trades | win_rate | PF | net_pnl | max_dd")
    for threshold, threshold_price in thresholds_price.items():
        run_dir = output_root / threshold
        run_dir.mkdir(parents=True, exist_ok=True)

        threshold_pips = threshold_price / pip_size
        strategy_cfg = dict(base_strategy_cfg)
        # Robustness sweep: only vary threshold, keep the rest frozen.
        strategy_cfg["impulse_threshold_pips"] = threshold_pips
        strategy_cfg["retracement_entry_ratio"] = float(args.entry_ratio)
        ...
        print(
            f"{threshold:>9} | {row['total_trades']:>6} | {row['win_rate']:.4f} | "
    ...
    summary = {
        ...
        "input": args.input,
        "fixed_entry_retracement_ratio": float(args.entry_ratio),
        "results": results,
    }
