...
REFERENCE_START = time(0, 0)
REFERENCE_END = time(7, 0)
SWEEP_START = time(7, 0)
SWEEP_END = time(10, 0)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze session liquidity sweep reversal behavior on EURUSD M15 bars."
    )
    parser.add_argument(
        "--bars",
    ...
    parser.add_argument(
        "--output-dir",
        default="outputs/session_liquidity_sweep_reversal_diagnostic",
...
def compute_daily_metrics(bars: pd.DataFrame) -> pd.DataFrame:
    ...
    for date, day in df.groupby("date", sort=True):
        reference = day.loc[_window_mask(day["tod"], REFERENCE_START, REFERENCE_END)]
        sweep = day.loc[_window_mask(day["tod"], SWEEP_START, SWEEP_END)]
        if reference.empty or sweep.empty:
            continue

        ref_high = float(reference["mid_high"].max())
        ref_low = float(reference["mid_low"].min())
        ref_range = ref_high - ref_low

        above_hits = sweep.loc[sweep["mid_high"] > ref_high, "timestamp"]
        below_hits = sweep.loc[sweep["mid_low"] < ref_low, "timestamp"]
        ...
        post = sweep.loc[sweep["timestamp"] >= sweep_time]
        if direction == "above":
        ...
        if ref_range > 0:
            reversal_ratio = reversal_move / ref_range
            follow_through_ratio = continuation_move / ref_range
        else:
            reversal_ratio = float("nan")
            follow_through_ratio = float("nan")
...
def build_distribution(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    for metric in ["reversal_ratio", "follow_through_ratio"]:
        ...
        for label, q in [("p10", 0.10), ("p25", 0.25), ("p50", 0.50), ("p75", 0.75), ("p90", 0.90)]:
            stats.append({"metric": metric, "stat": label, "value": _safe_q(s, q)})
...
def build_summary(daily: pd.DataFrame, dataset_path: str) -> dict[str, object]:
    ...
    return {
        ...
        "windows_utc": {
            "reference": {
                "start": REFERENCE_START.strftime("%H:%M"),
                "end_exclusive": REFERENCE_END.strftime("%H:%M"),
            },
            "sweep": {
                "start": SWEEP_START.strftime("%H:%M"),
                "end_exclusive": SWEEP_END.strftime("%H:%M"),
            },
        },
        ...
        "median_reversal_ratio": _safe_q(swept["reversal_ratio"], 0.50),
        "p75_reversal_ratio": _safe_q(swept["reversal_ratio"], 0.75),
        "median_follow_through_ratio": _safe_q(swept["follow_through_ratio"], 0.50),
