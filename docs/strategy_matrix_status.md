...
| tsmom_ma_cross | trend / momentum | rejected | walk_forward_validation | docs/experiments/tsmom_initial_results.md | outputs/trend_walk_forward/tsmom_ma_cross/ | daily MA crossover showed positive OOS PF on EURUSD and GBPUSD, but failed current promotion gates on trade count, drawdown, and concentration |
| tsmom_donchian | trend / momentum | rejected | walk_forward_validation | docs/experiments/tsmom_initial_results.md | outputs/trend_walk_forward/tsmom_donchian/ | Donchian breakout had the thinnest sample and failed OOS under the formal gates |
| tsmom_return_sign | trend / momentum | rejected | walk_forward_validation | docs/experiments/tsmom_initial_results.md | outputs/trend_walk_forward/tsmom_return_sign/ | return-sign momentum improved materially after sweep selection, but still failed the promotion drawdown and concentration gates |
