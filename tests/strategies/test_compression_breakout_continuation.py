...
def _config(
    *,
    ...
    breakout_close_strength_threshold: float = 0.7,
    stop_atr_multiple: float = 1.0,
    target_atr_multiple: float = 1.5,
    max_holding_bars: int = 8,
    one_trade_per_day: bool = False,
) -> CompressionBreakoutContinuationConfig:
    return CompressionBreakoutContinuationConfig(
        timeframe="15m",
        ...
        breakout_close_strength_threshold=breakout_close_strength_threshold,
...
def test_no_signal_when_not_compressed() -> None:
    strategy = CompressionBreakoutContinuationStrategy(_config(compression_threshold=0.8))
    bars = [
        ...
        _bar("2024-01-02 00:45:00", mid_open=1.1003, mid_high=1.1019, mid_low=1.1005, mid_close=1.1018),
    ]
    order = None
    for bar in bars:
...
def test_no_signal_when_breakout_not_strong_enough() -> None:
    strategy = CompressionBreakoutContinuationStrategy(_config())
    bars = [
        _bar("2024-01-02 00:00:00", mid_open=1.1000, mid_high=1.1002, mid_low=1.1000, mid_close=1.1001),
        _bar("2024-01-02 00:15:00", mid_open=1.1001, mid_high=1.1003, mid_low=1.1001, mid_close=1.1002),
        _bar("2024-01-02 00:30:00", mid_open=1.1002, mid_high=1.1004, mid_low=1.1002, mid_close=1.1003),
        _bar("2024-01-02 00:45:00", mid_open=1.1003, mid_high=1.1008, mid_low=1.1000, mid_close=1.10042),
    ]
    order = None
    for bar in bars:
        order = strategy.generate_order(bar, has_open_position=False, has_pending_order=False)
...
def test_long_signal_when_compressed_and_strong_upside_breakout() -> None:
    strategy = CompressionBreakoutContinuationStrategy(_config())
    bars = [
        _bar("2024-01-02 00:00:00", mid_open=1.1000, mid_high=1.1002, mid_low=1.1000, mid_close=1.1001),
        _bar("2024-01-02 00:15:00", mid_open=1.1001, mid_high=1.1003, mid_low=1.1001, mid_close=1.1002),
        _bar("2024-01-02 00:30:00", mid_open=1.1002, mid_high=1.1004, mid_low=1.1002, mid_close=1.1003),
        _bar("2024-01-02 00:45:00", mid_open=1.1003, mid_high=1.1009, mid_low=1.1004, mid_close=1.10085),
    ]
    order = None
    for bar in bars:
        order = strategy.generate_order(bar, has_open_position=False, has_pending_order=False)
...
def test_short_signal_when_compressed_and_strong_downside_breakout() -> None:
    strategy = CompressionBreakoutContinuationStrategy(_config())
    bars = [
        ...
        _bar("2024-01-02 00:45:00", mid_open=1.2003, mid_high=1.20015, mid_low=1.1998, mid_close=1.19985),
    ]
    order = None
    for bar in bars:
        order = strategy.generate_order(bar, has_open_position=False, has_pending_order=False)
...
def test_stop_and_target_initialized_correctly() -> None:
    ...
    assert order.stop_loss == pytest.approx(order.entry_reference - 0.0006)
    assert order.take_profit == pytest.approx(order.entry_reference + 0.0009)
